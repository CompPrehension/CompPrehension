...
//        qr.setAllowedLaws(List.of());
        ...
        qr = qr.toBuilder().targetLaws(countNextTargetLaws(allLaws, domain, countOfLaw)).build();
...
//        }
    ...
    public Decision decide(long exerciseAttemptId) {
        ...
        if(grade(exerciseAttemptId, null) >= countTargetGrade() && isAllLawsUsedWindowCount(exerciseAttempt)){
    ...
    private int classifyStudent(ExerciseAttemptWithQuestionsData exerciseAttempt){
        int result = 0;
    ...
    private boolean isAllLawsUsedWindowCount(ExerciseAttemptWithQuestionsData exerciseAttempt) {
    ...
    protected HashMap<String, List<Boolean>> getTargetLawsInteractions(ExerciseAttemptWithQuestionsData exerciseAttempt, int removeLastCount){
        List<AttemptQuestionData> allQuestions = new ArrayList<>();
        allQuestions.addAll(exerciseAttempt.questions());
    ...
    private HashMap<String, List<Boolean>> basicLawsUsage(ExerciseAttemptWithQuestionsData exerciseAttempt){
            ...
            for (AttemptQuestionInteractionData currentInteraction : allInteractions) {
                ...
                for (String currentViolation : allViolations) {
                    ...
                    if(allLawsUsage.containsKey(currentViolation)){
                        allLawsUsage.get(currentViolation).add(Boolean.FALSE);
                    }
                ...
                for (String currentLaw : allLaws) {
                    ...
                    if (allLawsUsage.containsKey(currentLaw)) {
                        allLawsUsage.get(currentLaw).add(Boolean.TRUE);
                    }
