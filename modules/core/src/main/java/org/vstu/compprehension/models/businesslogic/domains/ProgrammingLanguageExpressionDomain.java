...
//    }
    ...
    public Question createQuestionFromModel(String questionName, Model model, AbstractRdfStorage rs) {
        ...
        metadata.setDraft(true);  // ordinary metadata instance but this may be useful to indicate it's still "draft", i.e. not yet accepted for import to main table.
        question.setMetadata(metadata);

        metadata.setName(questionName);
        metadata.setDomainShortname(this.getShortName());
        metadata.setStage(AbstractRdfStorage.STAGE_READY);  // 3 = generated question
        metadata.setVersion(AbstractRdfStorage.GENERATOR_VERSION);  // v10 : generated by Domain
        metadata.setUsedCount(0L);
        metadata.setDateLastUsed(new Date()); // generated at this time
